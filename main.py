...
while game_is_on:
    time.sleep(ball.move_speed) #to slow down or fast the ball animation
    screen.update()
    ball.move()
    ...
    if ball.distance(r_paddle) < 50 and ball.xcor() > 300 or ball.distance(l_paddle) < 50 and ball.xcor() < -320 :
        ball.bounce_x()
